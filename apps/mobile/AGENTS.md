...
### Dev-menu floating debug button (disabled)

`plugins/with-dev-menu-fab.js` sets `EXDevMenuShowFloatingActionButton=false` (Info.plist + Android manifest meta-data): the default-ON floating button sat in the top-right corner, overlaying app UI and intercepting taps there (it made e2e flows tap it instead of app buttons). The dev menu stays reachable via ⌘D / shake / three-finger long-press; a user can still re-enable the button in the dev-menu settings (that per-install preference wins over the default).
