...
/** App locale mapped to an Intl locale string accepted by `Intl.*` formatters. */
 ...
 * symbol ("USD · $") as the secondary line. Mirrors the web Settings currency
 * list, derived from `Intl.DisplayNames` so adding a currency needs no edit here.
 */
export function currencyOptions(
  ...
  return AVAILABLE_CURRENCIES.map((code) => ({
    ...
    description: `${code} · ${currencySymbol(code, locale)}`,
  }))
}
