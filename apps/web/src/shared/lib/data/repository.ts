...
// (see repository-i18n.ts); the optional `apiCode` preserves the backend's exact
// machine code (e.g. `ACCOUNT_IN_USE`) so callers can switch on it instead of
// guessing from HTTP status.
