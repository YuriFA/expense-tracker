...
func (s *Server) RegisterUser(ctx context.Context, req api.RegisterUserRequestObject) (api.RegisterUserResponseObject, error) {
	res, err := s.auth.Register(ctx, string(req.Body.Email), req.Body.Password)
	if err != nil {
		writeDomainError(ginCtx(ctx), s.log, err)
		return nil, nil
	}
	cookieStr := cookie.BuildSession(s.cfg.SessionConfig, res.SessionID, int(s.cfg.SessionConfig.TTL.Seconds())).String()
...
func (s *Server) ResendVerification(ctx context.Context, _ api.ResendVerificationRequestObject) (api.ResendVerificationResponseObject, error) {
	user := s.currentUser(ctx)
...
func (s *Server) RequestPasswordReset(ctx context.Context, req api.RequestPasswordResetRequestObject) (api.RequestPasswordResetResponseObject, error) {
...
func (s *Server) ConfirmPasswordReset(ctx context.Context, req api.ConfirmPasswordResetRequestObject) (api.ConfirmPasswordResetResponseObject, error) {
