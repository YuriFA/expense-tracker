...
Every debtor and every debt operation SHALL belong to exactly one user.
Reading, updating, or deleting another user's debtor or debt operation
SHALL behave as if it does not exist (not-found). Debtor names SHALL be
unique per user among non-deleted debtors; a duplicate name for the same
user SHALL be rejected with an already-exists error.

#### Scenario: Duplicate debtor name for the same user

- **WHEN** the user creates a debtor named "Анна" and already has a non-deleted debtor named "Анна"
- **THEN** the request is rejected with an already-exists error

#### Scenario: Same debtor name for different users

- **WHEN** two different users each create a debtor named "Анна"
- **THEN** both debtors are created independently

#### Scenario: Another user's debtor is invisible

- **WHEN** a request addresses a debtor id owned by a different user
