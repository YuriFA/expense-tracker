...
- [ ] 2.5 Commit `.env.production.example` (placeholders only); confirm no
      secret values land in the repo

## 3. CI deploy pipeline

- [ ] 3.1 Extend `.github/workflows/deploy.yml`: build+push **both**
      images (api + web) with `sha-<short>` and `main` tags; trigger on
      `push: branches: [main]` + `workflow_dispatch` with optional
      `image_tag` input
- [ ] 3.2 Deploy job: SSH (`VPS_HOST`/`VPS_USER`/`VPS_SSH_KEY` secrets) →
...
- [ ] 4.1 `docs/deployment.md`: prerequisites (gateway present, DNS A
      record), first boot (network, `.env`, up), redeploy, rollback,
      trusted-proxy IP verification step, logs/health inspection
- [ ] 4.2 Cross-reference zvonok's gateway plan for the first-gateway-boot
      case (fresh VPS where `~/gateway` is not up yet)

## 5. Gates

- [ ] 5.1 Repo gates unaffected: `pnpm -C apps/web build` clean (image
