...
- [ ] 1.1 Create `apps/mobile/src/pages/login/model/schema.ts`: Zod schema
...
- [ ] 1.2 Rewrite `login-screen.tsx` around
...
- [ ] 1.3 Add `login-screen.test.tsx` (mocked `useAuth`): empty submit is
...
- [ ] 1.4 Delete the `react-hook-form`/`zod`/`@hookform/resolvers`
...
- [ ] 2.1 Create `apps/mobile/src/pages/register/model/schema.ts`: email/
...
- [ ] 2.2 Rewrite `register-screen.tsx` with the same structure as task 1.2
...
- [ ] 2.3 Add `register-screen.test.tsx`: mismatched confirm blocks submit
...
- [ ] 3.1 Create `apps/mobile/src/pages/accounts/model/schema.ts`: `name`
...
- [ ] 3.2 Split `new-account-sheet.tsx` into presentation container +
...
- [ ] 3.3 Add `new-account-form.test.tsx` (reuse the provider harness):
...
- [ ] 4.1 Create `apps/mobile/src/pages/dashboard/model/schema.ts`: `name`
  trimmed non-empty; `type`/`icon`/`color` with today's defaults and no
  error paths; `NewCategoryFormValues` via `z.infer`.
- [ ] 4.2 Split `new-category-sheet.tsx` into container +
...
- [ ] 4.3 Add `new-category-form.test.tsx`: empty name blocks submit with
...
- [ ] 5.1 Create `apps/mobile/src/features/create-transaction/model/schema.ts`:
...
- [ ] 5.2 Create field components in
...
- [ ] 5.3 Create `new-transaction-form.tsx` with
...
- [ ] 5.4 Slim `new-transaction-sheet.tsx` down to the presentation
...
- [ ] 5.5 Rewrite/extend `new-transaction-sheet.test.tsx`: amount `'0'`
...
- [ ] 6.1 Reconcile `apps/mobile/docs/conventions/forms.md` with the real
  ...
  omit `disabled`).
- [ ] 6.2 Only if Jest hits ESM transform failures on the new imports:
...
- [ ] 6.3 Record for the archive step: after this change archives, update
...
- [ ] 7.1 Review the existing flows that exercise migrated forms
...
- [ ] 7.2 Extend `07-add-expense.yaml` to cover submit-driven validation
...
- [ ] 7.3 Extend `05-add-account.yaml` the same way for the error
...
- [ ] 8.1 In `apps/mobile`: `pnpm type-check`, `pnpm lint`, `pnpm format`
  all pass.
- [ ] 8.2 In `apps/mobile`: `pnpm test` passes with the new and rewritten
  form tests.
- [ ] 8.3 From the workspace root: `pnpm knip` passes with no tolerance
  entry for the three form packages.
- [ ] 8.4 In `apps/mobile`: `pnpm test:e2e` passes strictly via the script
  (never bare `maestro test`) with the updated flows from group 7; the
  `TODO(sheet-dismiss)` workaround in flows is not weakened.
- [ ] 8.5 In `apps/mobile`: `pnpm exec expo export --platform ios`
  succeeds.
- [ ] 8.6 Final sweep: no hand-rolled form state remains — none of the five
