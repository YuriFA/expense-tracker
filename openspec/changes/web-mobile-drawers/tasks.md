...
- [ ] 1.1 Create `apps/web/src/shared/ui/drawer/` on reka-ui Drawer parts (no
      ...
      classes)
- [ ] 1.2 Create `apps/web/src/shared/ui/responsive-dialog/`:
...
- [ ] 2.1 Migrate `features/transaction/add/ui/AddTransactionDialogHost.vue`
      to `responsive-dialog`
- [ ] 2.2 Migrate `features/transaction/add/ui/NewCategoryDialog.vue` and
      verify dialog-in-drawer stacking
- [ ] 2.3 Pilot checklist at 390×844 (browser + PWA standalone): stacked
...
- [ ] 3.1 `features/transaction/edit/ui/EditTransactionDialog.vue` (migrate
...
- [ ] 3.3 Debts: `pages/debts/ui/DebtorFormDialog.vue`,
...
- [ ] 3.6 `features/household-join/ui/HouseholdChoiceDialog.vue`,
      `features/sync-conflicts/ui/ConflictCenter.vue`,
      `pages/analytics-detail/ui/CategoryCashflowDialog.vue`
- [ ] 3.7 `widgets/mobile-shell/ui/SpeedDialFab.vue` per-kind add dialogs
- [ ] 3.8 Exempt surfaces stay untouched: the 6 AlertDialogs
...
- [ ] 4.1 `pages/transactions/ui/TransactionsFiltersSheet.vue`: below 768px a
      bottom drawer (drop the `sm:max-w-[400px]` right-panel sizing on
      mobile), keep the right-side Sheet at 768px+

## 5. Tests

- [ ] 5.1 Unit tests for `drawer/`, `responsive-dialog/`, the responsive
      select variant, and `date-field/` (Vitest + mount-with-providers;
      Teleport-aware mounting for drawers)
- [ ] 5.2 Update existing dialog test files affected by the import swap;
      desktop-presentation assertions keep passing
- [ ] 5.3 e2e: update backendless + PWA specs that assert dialog presentation
...
- [ ] 6.1 `apps/web/docs/conventions/vue-patterns.md` §4: overlay conventions
      (container owns presentation/lifecycle, form owns state;
      responsive-dialog usage; the exemption list)
- [ ] 6.2 `apps/web/docs/ARCHITECTURE.md`: shared/ui list + the
      viewport-aware overlay decision
- [ ] 6.3 Update code comments citing the superseded D1
      (`pages/debts/ui/DebtorHistoryDialog.vue:29`,
      `pages/analytics-detail/ui/CategoryCashflowDialog.vue:31`)
- [ ] 6.4 i18n keys for any new a11y labels (`packages/i18n` ru/en) +
      `pnpm i18n:lint`

## 7. Gates & finalize

- [ ] 7.1 Checks: web type-check, oxlint+eslint, `pnpm test:unit`,
